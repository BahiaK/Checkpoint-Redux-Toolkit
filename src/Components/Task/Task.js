...
const Task = ({el}) => {
  ...
  return (
    ...
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-around'}} >
      ...
      <div> {edit?<input type="text" Value={el.description} onChange={(e)=>setDeskEdit(e.target.value)} />:<h1  style={{color :`Black`}}>{el.description}</h1>}</div>
      <div>
        
      <button class="btn btn-primary"onClick={()=>dispatch(changeStatus(el.id))} >{el.isDone? "Done":"unDone"}</button>
